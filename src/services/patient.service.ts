...
export async function uploadMyPhoto(uri: string): Promise<{ photo_url: string }> {
  const formData = new FormData();
  await api.appendFilePart(formData, 'photo', { uri, name: 'photo.jpg', mimeType: 'image/jpeg' });
...
export async function uploadMyExam(data: NewExamInput): Promise<void> {
  const formData = new FormData();
...
export async function analyzeMyExam(file: { uri: string; name: string; mimeType: string }): Promise<OcrResult> {
  const formData = new FormData();
