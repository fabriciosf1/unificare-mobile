...
export default function FamilyAddExamScreen({ onBack, onSaved }: { onBack: () => void; onSaved: () => void }) {
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    try {
      await uploadFamilyExam({ exam_type: examType, exam_date: date, observations: observations || undefined, lab_name: labName, structured_data: structuredData, file });
      setSentCount((c) => c + 1);
      handleDiscard();
  ...
  if (showCamera) {
    return <DocumentCameraScreen onCancel={() => setShowCamera(false)} onCaptured={handleCaptured} accentColor={colors.blue} />;
  }
